...
const defaultColor='rgb(41, 122, 255)'
 class Home extends Component {
     ...
     componentDidUpdate(prevProps,prevState){
        if(prevProps.question.loading !== this.props.question.loading){
            this.setState({question:nextProps.question})
        }
        if(prevState.question !== this.state.question){
            this.setState({timer:this.state.question})
        }
     }

     asdasd=()=>{
         setTimeout(() => this.setState(prev => ({timer:prev.timer-1})),999)
     } 
    render() {
                ...
                <Content>
                    <View style={{flex:1,width:'100%',padding:10}}>
                        <View style={{flex:1,width:'100%',alignItems:'center',justifyContent:'center',height:120,padding:10}}>
                            <Text style={{fontSize:22}}>PT finalTest {this.state.timer}</Text>
                        </View>
                        ...
                        <View style={{flex:1,width:'100%',alignItems:'center',justifyContent:'center',height:120,padding:10}}>
                            <Button rounded onPress={() => this.register()} style={{backgroundColor:'rgb(120, 172, 255)',alignSelf:'center',marginTop:30}}>
                                <Text>Mengerti</Text>
                            </Button>
                        </View>
                    </View>
